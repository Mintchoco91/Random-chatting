...
public class InsertLoginTaskRxjava {
    ...
    private void resultPost(com.example.random_chatting.TaskDTO.OutputDTO outputDTO){
        //Insert 일경우
        if(outputDTO.getMode().equals(com.example.random_chatting.TaskDTO.OutputDTO.modeType.INSERT)) {
    ...
    public void backgroundTask(String... params) {
        ...
        backgroundTask = Observable.fromCallable(() -> {
            String result = null;
                    com.example.random_chatting.TaskDTO.OutputDTO outputDTO = new com.example.random_chatting.TaskDTO.OutputDTO();
            //doInBackground(task에서 실행할 코드 여기에 작성)
            //params[2] = mode
            if(params[2].equals("insert")){
                result = insertFunc(params);
                outputDTO.setResult(result);
                outputDTO.setMode(com.example.random_chatting.TaskDTO.OutputDTO.modeType.INSERT);
            }
            return outputDTO;
